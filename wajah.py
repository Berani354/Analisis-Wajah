...
# Fungsi utama
def main():
    add_css()  # Tambahkan CSS untuk sidebar

    with st.sidebar:
        menu_id = option_menu(
            "Navigasi",
            ["Home", "Unggah Gambar", "Analisis Fitur", "Tentang Aplikasi"],
        ...
        st.image("buatkan gambar ai tentang analisis wajah.png", 
    ...
    if menu_id == "Home":
        logo_path = "logo udw.png"
        try:
            st.image(logo_path, caption="Logo Universitas Dharma Wacana", use_container_width=True)
        except Exception:
            st.error("Error membuka gambar logo: Pastikan file tersedia di lokasi yang benar.")
  # Deskripsi aplikasi
    ...
    elif menu_id == "Unggah Gambar":
        upload_page()

    elif menu_id == "Analisis Fitur":
        analysis_page()

    elif menu_id == "Tentang Aplikasi":
        about_page()
